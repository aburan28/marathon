...
def test_launch_group():
    ...
    client = marathon.create_client()
    client.create_group(common.group())
...
# XXX: find out why the task never becomes unhealthy
def test_failing_health_check_results_in_unhealthy_app():
    ...
    app_def = apps.http_server()
    app_def['healthChecks'] = [common.health_check('/bad-url', 'HTTP', failures=0, timeout=0)]
    app_def['healthChecks'][0]['gracePeriodSeconds'] = 1
...
# XXX: make sure that the task becomes unhealthy
@shakedown.private_agents(2)
...
# XXX: sometimes two tasks are launched
